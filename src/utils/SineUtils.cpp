...
void playOscillator(std::vector<int16_t> &samples, Synth::Oscillator &osc,
  ...
  const int32_t totalSamples = static_cast<int>(osc.getSampleRate()) * duration;

  for (int32_t i = 0; i < totalSamples; ++i) {
    float value = amplitude * osc.getNextSampleValue();
...
void playOscChord(std::vector<int16_t> &samples,
  ...
  for (int32_t i = 0; i < totalSamples; ++i) {
    ...
    // Convert from floating point (-1.0 to 1.0) to 16-bit integer (-32768 to
    // 32767) This is how audio is stored in WAV files
    int16_t sample = static_cast<int16_t>(relativeAmplitude * value * 32767.0);
    samples.push_back(sample);
  }
}
